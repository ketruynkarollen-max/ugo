...
(function () {
  ...
  function render() {
    ...
    const polygon = skills.map(([, v], i) => pt(i, v).join(',')).join(' ');
    ...
    <div class="card score-card">
      <div style="font-size:13px;color:var(--ink-3);font-weight:600">${td('ss.scoreTitle')}</div>
      <div style="position:relative;width:170px;height:170px">
        ...
        <div style="position:absolute;inset:0;display:grid;place-items:center;flex-direction:column">
          <div style="text-align:center">
            ...
            <div style="font-size:12px;color:var(--ink-3);margin-top:6px;font-weight:500">${td('ss.veryGood')}</div>
          </div>
        </div>
      </div>
      <div class="delta delta--up" style="font-size:13px"><svg class="btn-icon"><use href="#i-arrow-up"/></svg> ${td('ss.vsMonth')}</div>
    </div>

    <div class="card" style="padding:24px;display:flex;flex-direction:column">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
        <div style="font-size:14px;font-weight:700;color:var(--ink)">${td('ss.evoTitle')}</div>
        <div style="display:flex;gap:10px;font-size:11px">
          <span style="display:flex;align-items:center;gap:4px;color:var(--ink-3)"><span style="width:8px;height:8px;border-radius:50%;background:#22C55E"></span>${td('ss.legendLow')}</span>
          <span style="display:flex;align-items:center;gap:4px;color:var(--ink-3)"><span style="width:8px;height:8px;border-radius:50%;background:#FF6A1A"></span>${td('ss.legendMid')}</span>
          <span style="display:flex;align-items:center;gap:4px;color:var(--ink-3)"><span style="width:8px;height:8px;border-radius:50%;background:#2F6BFF"></span>${td('ss.legendHigh')}</span>
        </div>
      </div>
      <div style="flex:1;display:grid;place-items:center;margin-top:8px">
        <svg viewBox="-50 -30 400 370" style="width:100%;max-width:380px;height:auto">
          ...
          <text x="${cx}" y="${cy + 12}" text-anchor="middle" font-size="9" fill="#6B6B70">${td('ss.avgScore')}</text>
          ${labels}
        </svg>
      </div>
    </div>
    ...
    <div class="card" style="padding:18px 20px">
      <div style="font-size:13px;color:var(--ink-3);font-weight:600;margin-bottom:6px">${td('ss.tableTitle')}</div>
      <table class="softskills-table">
        <thead>
          <tr><th>${td('ss.thSkill')}</th><th>${td('ss.thScore')}</th><th>${td('ss.thEvo')}</th></tr>
        </thead>
        <tbody>
          ${skills
            .map(
